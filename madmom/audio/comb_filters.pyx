...
def feed_backward_comb_filter(signal, tau, alpha):
    ...
    """
    if signal.ndim == 1:
        return _feed_backward_comb_filter_1d(signal.astype(np.float64),
                                             tau, alpha)
    elif signal.ndim == 2:
        return _feed_backward_comb_filter_2d(signal.astype(np.float64),
                                             tau, alpha)
    else:
...
def comb_filter(signal, filter_function, tau, alpha):
    ...
    alpha = np.array(alpha, dtype=np.float64, ndmin=1)
