...
def hz2midi(f, fref=A4):
    ...
    """
    return (12. * np.log2(np.asarray(f, dtype=np.float64) / fref)) + 69.
...
def midi2hz(m, fref=A4):
    ...
    """
    return 2. ** ((np.asarray(m, dtype=np.float64) - 69.) / 12.) * fref
...
def bins2frequencies(bins, bin_frequencies):
    ...
    return np.asarray(bin_frequencies, dtype=np.float64)[np.asarray(bins)]
...
class Filterbank(np.ndarray):
    ...
    def __new__(cls, data, bin_frequencies):
        ...
        obj.bin_frequencies = np.asarray(bin_frequencies, dtype=np.float64)
        # return the object
        return obj
