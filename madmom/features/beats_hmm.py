...
class MultiPatternStateSpace(object):
    ...
    def __init__(self, state_spaces):
        ...
        self.state_positions = np.empty(0)
        self.state_intervals = np.empty(0, dtype=np.int64)
        self.state_patterns = np.empty(0, dtype=np.int64)
        self.num_states = 0
...
class RNNBeatTrackingObservationModel(ObservationModel):
    ...
    def log_densities(self, observations):
        ...
        """
        # init densities
        log_densities = np.empty((len(observations), 2), dtype=np.float64)
...
class RNNDownBeatTrackingObservationModel(ObservationModel):
    ...
    def log_densities(self, observations):
        ...
        """
        # init densities
        log_densities = np.empty((len(observations), 3), dtype=np.float64)
...
class GMMPatternTrackingObservationModel(ObservationModel):
    ...
    def log_densities(self, observations):
        ...
        # init the densities
        log_densities = np.empty((len(observations), num_gmms), dtype=np.float64)
